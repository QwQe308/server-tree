addLayer("p2", {
    name: "prestige layer 2", // This is optional, only used in a few places, If absent it just uses the layer id.
    symbol: "P^2", // This appears on the layer's node. Default is the id with the first letter capitalized
    ...
    color: "green",
    resource: "重置^2点", // Name of prestige currency
    baseResource: "点数",
    baseAmount() {return player.p1.points},
    requires(){return new ExpantaNum(3125)},
    exponent: 1,
    ...
    gainExp() { // Calculate the exponent on main currency from bonuses
        var exp = n(1/Number(this.layer[1])**2)
    ...
    layerShown(){return hasUpgrade("p1",21)||player[this.layer].unlocked},
    //effect(){
    ...
    upgrades: {
        11: {
            title:"重置加成",
            description: "本层重置点加成点数获取.该加成不会低于2.",
            cost(){return n(1)},
            unlocked(){return true},
            effect(){
                var baseEff = player[this.layer].points.add(1).log10().add(1).pow(4).mul(2)
                if(hasUpgrade(this.layer,15)) baseEff = baseEff.pow(upgradeEffect(this.layer,15))
                if(hasUpgrade("p4",21)) baseEff = baseEff.pow(baseEff.pow(0.8).log10().cbrt().add(1))
                return baseEff
            },
        ...
        14: {
            title:"双子星系",
            description: "如果你的本层重置点的整数部分是偶数,点数x69.",
            cost(){return n(125)},
            unlocked(){return hasUpgrade(this.layer,this.id-1)},
            effect(){
        ...
        21: {
            ...
            unlocked(){return hasUpgrade(this.layer,15)||hasUpgrade(this.layer,this.id)},
        ...
        24: {
            title:"双子银河",
            description: "如果你的该层的重置点的指数的整数部分是偶数,点数x69420^(本层层级数^0.5).升级14效果永久触发.低层的“双子银河”永久生效。",
            cost(){return n(1e7).pow(Number(this.layer[1])**0.33)},
            unlocked(){return hasUpgrade(this.layer,this.id-1)},
            effect(){
                var p = player[this.layer].points.max(1).log10().floor()
                if(p.div(2).floor().eq(p.div(2)) || hasUpgrade(`p${Number(this.layer[1])+1}`,24)) var baseEff = n(69420).pow(Number(this.layer[1])**0.5)
                else baseEff = n(1)
                return baseEff
            },
            effectDisplay(){return `x${format(upgradeEffect(this.layer,this.id),1)}`}
        ...
        25: {
            ...
            unlocked(){return hasUpgrade(this.layer,this.id-1)},
            effect(){
                var baseEff = player[`p${Number(this.layer[1])+1}`].points.add(10).log10().pow(2.56).pow(Number(this.layer[1]))
                return baseEff
            },
    ...
    */
    ...
    hotkeys: [
        {key: "2", description: "2: PL2转", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
    ],
    passiveGeneration(){
        if(hasUpgrade("p4",11)) return 0.1
        return 0
    },
    doReset(layer){
        if(layers[layer].position>Number(this.layer[1])){
            var kp = []
            if(hasUpgrade(`p3`,21)) kp.push("upgrades")
            layerDataReset(this.layer, kp)
            if(!hasUpgrade(`p3`,21)) player[this.layer].upgrades = [21]
        }
    }
    /*tabFormat: {
        Token挑战表: {

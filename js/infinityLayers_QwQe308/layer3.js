addLayer("p3", {
    name: "prestige layer 3", // This is optional, only used in a few places, If absent it just uses the layer id.
    symbol: "P^3", // This appears on the layer's node. Default is the id with the first letter capitalized
    ...
    color: "green",
    resource: "重置^3点", // Name of prestige currency
    baseResource: "点数",
    ...
    layerShown(){return hasUpgrade("p2",21)||player[this.layer].unlocked},
    //effect(){
    ...
    upgrades: {
        11: {
            title:"重置加成",
            description: "本层重置点加成点数获取.该加成不会低于2.自动获取10%P1.",
            cost(){return n(1)},
        ...
        12: {
            ...
            effect(){
                var baseEff = player.points.add(10).log10().pow(2).pow(Number(this.layer[1])**0.5)
                if(hasUpgrade(this.layer,15)) baseEff = baseEff.pow(upgradeEffect(this.layer,15))
                return baseEff
            },
            effectDisplay(){return `x${format(upgradeEffect(this.layer,this.id),1)}`}
        ...
        13: {
            ...
            unlocked(){return hasUpgrade(this.layer,this.id-1)},
            effect(){
                var baseEff = player[this.layer].bestP.add(10).log10().pow(1.14514).pow(Number(this.layer[1])**0.65)
                if(hasUpgrade(this.layer,15)) baseEff = baseEff.pow(upgradeEffect(this.layer,15))
                return baseEff
            },
            effectDisplay(){return `x${format(upgradeEffect(this.layer,this.id),1)}`}
        ...
        14: {
            title:"双子星系",
            description: "如果你的本层重置点的整数部分是偶数,点数x69^(本层层级数^0.75).",
            cost(){return n(125)},
            unlocked(){return hasUpgrade(this.layer,this.id-1)},
            effect(){
                var p = player[this.layer].points.floor()
                if(p.div(2).floor().eq(p.div(2)) || hasUpgrade(this.layer,24)) var baseEff = n(69).pow(Number(this.layer[1])**0.75)
                else baseEff = n(1)
                if(hasUpgrade(this.layer,15)) baseEff = baseEff.pow(upgradeEffect(this.layer,15))
                return baseEff
            },
        ...
        24: {
            title:"双子银河",
            description: "如果你的该层的重置点的指数的整数部分是偶数,点数x69420^(本层层级数^0.65).升级14效果永久触发.低层的“双子银河”永久生效。",
            cost(){return n(1e7).pow(Number(this.layer[1])**0.33)},
            unlocked(){return hasUpgrade(this.layer,this.id-1)},
            effect(){
                var p = player[this.layer].points.max(1).log10().floor()
                if(!player[`p${Number(this.layer[1])+1}`]){
                    if(p.div(2).floor().eq(p.div(2))){
                        var baseEff = n(69420).pow(Number(this.layer[1])**0.65)
                    } 
                }
                else if(p.div(2).floor().eq(p.div(2)) || hasUpgrade(`p${Number(this.layer[1])+1}`,24)){
                    var baseEff = n(69420).pow(Number(this.layer[1])**0.65)
                } 
                else baseEff = n(1)
                return baseEff
        ...
        25: {
            title:"降维打击",
            description: `高层重置点加成本层重置点获取.`,
            cost(){return n(1e7).pow(Number(this.layer[1])**0.5)},
            unlocked(){return hasUpgrade(this.layer,this.id-1)},
            effect(){
                if(player[`p${Number(this.layer[1])+1}`]) var baseEff = player[`p${Number(this.layer[1])+1}`].points.add(10).log10().pow(2.56).pow(Number(this.layer[1]))
                else baseEff = n(1)
                return baseEff
            },
    ...
    */
    ...
    hotkeys: [
        {key: "3", description: "3: PL3转", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
    ],
    passiveGeneration(){
        if(hasUpgrade("p4",25)) return 0.1
        return 0
    },
    doReset(layer){
        if(layers[layer].position>Number(this.layer[1])){
            var kp = []
            if(hasUpgrade(`p4`,21)) kp.push("upgrades")
            layerDataReset(this.layer, kp)
            if(!hasUpgrade(`p4`,21)) player[this.layer].upgrades = [21]
        }
    }
    /*tabFormat: {
        Token挑战表: {
